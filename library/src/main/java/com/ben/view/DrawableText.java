 ...
 * @link http://blog.csdn.net/jiaoyang623/article/details/7790494
 * */
public class DrawableText extends TextView {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.DrawableText, defStyle, 0);
        if (a != null) {
            // 获得参数个数
            ...
            for (int i = 0; i < count; i++) {
                index = a.getIndex(i);
                if (index == R.styleable.DrawableText_right_height) {
                    rightHeight = a.getDimensionPixelSize(index, -1);
                } else if (index == R.styleable.DrawableText_right_width) {
                    rightWidth = a.getDimensionPixelSize(index, -1);
                } else if (index == R.styleable.DrawableText_left_height) {
                    leftHeight = a.getDimensionPixelSize(index, -1);
                } else if (index == R.styleable.DrawableText_left_width) {
                    leftWidth = a.getDimensionPixelSize(index, -1);
                } else if (index == R.styleable.DrawableText_bottom_height) {
                    bottomHeight = a.getDimensionPixelSize(index, -1);
                } else if (index == R.styleable.DrawableText_bottom_width) {
                    bottomWidth = a.getDimensionPixelSize(index, -1);
                } else if (index == R.styleable.DrawableText_top_height) {
                    topHeight = a.getDimensionPixelSize(index, -1);
                } else if (index == R.styleable.DrawableText_top_width) {
